...
const EmotionDetectionContent = ({ api }) => {
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-gray-50 p-6 h-96 rounded-xl border-4 border-dashed border-orange-300 flex flex-col items-center justify-center mb-8" style={{ background: 'linear-gradient(180deg, #fff8f0, #f5f5f5)' }}>
        <div className="text-6xl text-orange-400 p-4 rounded-full bg-white mb-4"><span role="img" aria-label="smiley face">😊</span></div>
        <p className="text-gray-600 mb-4">Camera feed for emotion detection</p>
        <PrimaryButton onClick={handleStartDetection} className="bg-orange-600 hover:bg-orange-700 w-56" disabled={isDetecting || api.loading}>
          ...
          <div className="space-y-3">
            {Object.entries(emotions).map(([name, percent]) => (
              <div key={name} className="flex flex-col">
                <div className="flex justify-between text-sm font-medium text-gray-600 mb-1"><span>{name}</span><span>{percent.toFixed(0)}%</span></div>
                <div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-blue-500 h-2.5 rounded-full transition-all duration-500" style={{ width: `${percent}%` }}></div></div>
              </div>
            ))}
          </div>
        </div>
